...
drawGraphPlot4 <- function() {
	...
	par(mfcol=c(2,2))
	plot(df$DT1,df$Global_active_power, type="n",xlab="",ylab="Global Active Power")
	lines(df$DT1, df$Global_active_power)
	...
	plot(df$DT1,df$Voltage, type="n",xlab="datetime",ylab="Voltage")
	lines(df$DT1, df$Voltage)

	plot(df$DT1,df$Global_reactive_power, type="n",xlab="datetime",ylab="Global_reactive_power")
	lines(df$DT1, df$Global_reactive_power)
